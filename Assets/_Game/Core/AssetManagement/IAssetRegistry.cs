    ...
    namespace _Game.Core.AssetManagement
{
    ...
    {
        UniTask<T> LoadAsset<T>(AssetReference assetReference, int timeline, int cacheContext) where T : class;
        UniTask<T> LoadAsset<T>(string key, int timeline, int cacheContext) where T : class;
